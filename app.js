...
function buildPersonGrid() {
  ...
  state.bank.interviewers.forEach(function (p) {
    g.appendChild(optBtn(
      ...
      '<div class="pmeta"><b>' + p.name + '</b><small>' + p.title + '</small></div></div>',
      p.id === state.interviewer.id, function () { state.interviewer = p; pickVoice(); refresh(); }, 'person'));
  });
}
...
/* ---------- voices ---------- */
function loadVoices() {
  const all = speechSynthesis.getVoices().filter(function (v) { return v.lang.toLowerCase().startsWith('en'); });
  window._voices = all;
  const sel = $('voiceSelect');
  if (!all.length) { sel.innerHTML = '<option>No English voices found</option>'; return; }
  sel.innerHTML = '';
  all.forEach(function (v) { const o = document.createElement('option'); o.value = v.name; o.textContent = v.name + ' (' + v.lang + ')'; sel.appendChild(o); });
  sel.onchange = function () { state.voice = all.find(function (v) { return v.name === sel.value; }) || null; };
  pickVoice();
}
function pickVoice() {
  const all = window._voices || []; if (!all.length || !state.interviewer) return;
  const acc = (state.interviewer.accent || 'en-US').toLowerCase();
  const m = all.find(function (v) { return v.lang.toLowerCase().startsWith(acc); }) ||
            all.find(function (v) { return v.lang.toLowerCase().startsWith('en'); });
  if (m) { state.voice = m; const sel = $('voiceSelect'); if (sel) sel.value = m.name; }
}
...
function buildInterview() {
  ...
  const pool = src.map(function (q) { return q.text; });
...
function askCurrent() {
  ...
  speak(state.questions[state.idx]).then(function () {
...
function pressBig() {
  ...
  if (!state.answering) {
    ...
    setTimeout(function () {
      state.answers.push({
        q: state.questions[state.idx], a: state.ansText, secs: secs,
